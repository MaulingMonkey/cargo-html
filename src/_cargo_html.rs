...
fn main() {
    ...
    let template_script_js  = include_str!("../template/script.js");
    ...
    for config in configs.iter().copied() {
        ...
        for (_ty, target) in targets.iter() {
            ...
            let script_placeholder_idx = template_html.find(script_placeholder).expect("template missing {SCRIPT}");
            ...
            mmrbi::fs::write_if_modified_with(target_html, |o| {
                writeln!(o, "{}", &template_html[..script_placeholder_idx])?;
                writeln!(o, "{}", template_script_js)?;
                writeln!(o, "exec_base64_wasm({:?});", target_wasm)?;
                writeln!(o, "{}", &template_html[(script_placeholder_idx + script_placeholder.len())..])?;
                Ok(())
