namespace wasi {
    ...
    export function signals(i: Imports, _memory: MemoryLE, tty: XTermTty | DomTty | undefined, settings: Settings) {
        const trace_exit_0 = TextStreamWriter.from_output(settings.trace_exit_0 || settings.stdout || (tty ? "tty" : "console-log"  ), "exit-ok",    tty);
        const trace_exit_n = TextStreamWriter.from_output(settings.trace_exit_n || settings.stderr || (tty ? "tty" : "console-error"), "exit-error", tty);
        const trace_signal = TextStreamWriter.from_output(settings.trace_signal || settings.stderr || (tty ? "tty" : "console-error"), "exit-error", tty);
