declare type Terminal           = import('xterm').Terminal;
declare type ITerminalOptions   = import('xterm').ITerminalOptions;
...
class XTermTty {
    ...
    listen(listener: (data: string) => void) {
    ...
    shutdown() {
        this.terminal.write("\x1B[?25l"); // hide the cursor
        this.alive = false;
    }
    ...
    private constructor(settings: TtySettings) {
        ...
        // @ts-ignore
        if (typeof WebLinksAddon !== "undefined") this.terminal.loadAddon(new WebLinksAddon.WebLinksAddon());
