...
const WASMS = {};
function mount_wasm_base64(name, wasm) {
    WASMS[name] = wasm;
}
async function launch_wasm(name) {
    const wasm = WASMS[name];
    if (wasm === undefined)
        throw `launch_wasm(${JSON.stringify(name)}): no such wasm module mounted`;
    const settings = get_settings();
    if (document.getElementById("cargo-html-console"))
        settings.tty = settings.tty || { listen: document, output: "cargo-html-console", input: "cargo-html-console-input" };
...
class DomTty {
    ...
    vga(color, bright) {
        if (!bright)
            switch (color) {
                case Color.Black: return "var(--cargo-html-console-color-0)";
                case Color.Red: return "var(--cargo-html-console-color-1)";
                case Color.Green: return "var(--cargo-html-console-color-2)";
                case Color.Yellow: return "var(--cargo-html-console-color-3)";
                case Color.Blue: return "var(--cargo-html-console-color-4)";
                case Color.Magenta: return "var(--cargo-html-console-color-5)";
                case Color.Cyan: return "var(--cargo-html-console-color-6)";
                case Color.White: return "var(--cargo-html-console-color-7)";
            }
        else
            switch (color) {
                case Color.Black: return "var(--cargo-html-console-color-8)";
                case Color.Red: return "var(--cargo-html-console-color-9)";
                case Color.Green: return "var(--cargo-html-console-color-10)";
                case Color.Yellow: return "var(--cargo-html-console-color-11)";
                case Color.Blue: return "var(--cargo-html-console-color-12)";
                case Color.Magenta: return "var(--cargo-html-console-color-13)";
                case Color.Cyan: return "var(--cargo-html-console-color-14)";
                case Color.White: return "var(--cargo-html-console-color-15)";
            }
    }
}
function beep() {
...
class XTermTty {
    constructor(settings) {
        ...
        if (typeof FitAddon !== "undefined") {
            ...
            output.addEventListener("resize", () => fit.fit());
            console.log(output);
            this.terminal.loadAddon(fit);
        }
        this.terminal.open(output);
        this.terminal.focus();
    }
    static new(settings) {
    ...
    listen(listener) {
        ...
        this.terminal.onBinary(recv => listener(recv));
        this.terminal.onData(recv => listener(recv));
    }
    shutdown() {
...
(function (wasi) {
    class ConReader {
        ...
        static try_create(settings, tty) {
            if (tty === undefined)
                return undefined;
            try {
...
(function (wasi) {
    class TextStreamWriter {
        ...
        static from_output(output, color_hint, tty) {
            switch (output) {
                ...
                case "tty":
                    if (tty === undefined)
                        throw "output === \"dom\", but no tty was provided";
...
(function (wasi) {
    ...
    (function (fs_1) {
        class MemoryDirHandle {
            ...
            path_link(old_flags, old_path, new_handle, new_path) { this.path_link_or_rename(old_flags, old_path, new_handle, new_path, false); }
            path_rename(old_path, new_handle, new_path) { this.path_link_or_rename(wasi.LOOKUPFLAGS_NONE, old_path, new_handle, new_path, true); }
            path_link_or_rename(old_flags, old_path, new_handle, new_path, rename) {
                const old_handle = this;
                if (!(new_handle instanceof MemoryDirHandle))
                    throw wasi.ERRNO_XDEV;
                if (old_handle.fs !== new_handle.fs)
                    throw wasi.ERRNO_XDEV;
                let [old_parents, old_name] = this.path_parents_name(old_path);
                let [new_parents, new_name] = new_handle.path_parents_name(new_path);
                if (old_name === ".") {
                    const leaf = old_parents.pop();
                    const dir = old_parents[old_parents.length - 1];
                    const n = find_node(dir, leaf);
                    if (n === undefined)
                        throw wasi.ERRNO_IO;
                    old_name = n;
                }
                if (new_name === ".") {
                    const leaf = new_parents.pop();
                    const dir = new_parents[new_parents.length - 1];
                    const n = find_node(dir, leaf);
                    if (n === undefined)
                        throw wasi.ERRNO_IO;
                    new_name = n;
                ...
                if (!new_dir)
                    throw wasi.ERRNO_NOTDIR;
                if (!(old_name in old_dir.children))
                    throw wasi.ERRNO_NOENT;
        ...
        function throws(exception, expr) {
            try {
                expr();
                return false;
            }
        ...
        function sanitize_dirs_name(path) {
            const p = sanitize_path(path).split("/");
            const leaf = p.pop();
            return [p, leaf];
        }
        function find_node(dir, node) {
            if (!dir)
                return undefined;
            for (const name in dir.children)
                if (dir.children[name] === node)
                    return name;
            return undefined;
        }
    })(fs = wasi.fs || (wasi.fs = {}));
...
var wasi;
(function (wasi) {
    function fdio(i, memory, asyncifier, tty, settings) {
        const trace = true;
        ...
        const stdout = wasi.TextStreamWriter.from_output(settings.stdout || (tty ? "tty" : "console-log"), "stdout", tty);
        const stderr = wasi.TextStreamWriter.from_output(settings.stdout || (tty ? "tty" : "console-error"), "stderr", tty);
        if (stdout)
...
var wasi;
(function (wasi) {
    function signals(i, _memory, tty, settings) {
        const trace_exit_0 = wasi.TextStreamWriter.from_output(settings.trace_exit_0 || settings.stdout || (tty ? "tty" : "console-log"), "exit-ok", tty);
        const trace_exit_n = wasi.TextStreamWriter.from_output(settings.trace_exit_n || settings.stderr || (tty ? "tty" : "console-error"), "exit-error", tty);
        const trace_signal = wasi.TextStreamWriter.from_output(settings.trace_signal || settings.stderr || (tty ? "tty" : "console-error"), "exit-error", tty);
